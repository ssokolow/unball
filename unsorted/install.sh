...
NAUTILUS_SCRIPT_SUFFIX="" # Set to categorize Nautilus scripts hierarchically.

function install_nautilus() {
    # Usage: install_nautilus <UID> <GID> <HOMEDIR>
    # Purpose: Install unball and MoveToZip into the nautilus-actions menu

    if [ ! -d "$3/.gnome2/nautilus-scripts" ]; then return 1; fi

    # Make the script category dir if it doesn't exist
        NSCRIPT_PATH="$3/.gnome2/nautilus-scripts"
...
function user_enum() {
    ...
    for LINE in `cut -sd: -f3,4,6 /etc/passwd`; do
        ...
        $1 $LINE_SPLIT # DON'T quote $LINE_SPLIT. It must become 3 positional args
    done
}
